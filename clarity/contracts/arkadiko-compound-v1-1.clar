...
;; TODO - verify that is using the right contract token so should tie it to wstx-usda for example
...
(define-constant CONTRACT-STRATEGIST 'SP2XTTX8JDH033YBVH6D92HFM25Y2FGDE3F6HR4EH)
...
(define-public (stake (registry-trait <stake-registry-trait>) (pool-trait <stake-pool-trait>) (token-trait <ft-trait>) (amount uint) (should-transfer-token bool))
    (let (
...
(define-private (add-liquidity-to-dex (reward uint))
    (let (
        ...
        ;; Get dust after swap
        (dust-stx (stx-get-balance (as-contract tx-sender)))
        (dust-usda (unwrap-panic (as-contract (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token get-balance tx-sender))))
    )
        ...
        ;; Donate STX dust
        (if (> dust-stx u0)
            (try! (as-contract (stx-transfer? dust-stx tx-sender CONTRACT-STRATEGIST)))
            false
        )
        ;; Donate USDA dust
        (if (> dust-usda u0)
            (try! (as-contract (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token transfer dust-usda tx-sender CONTRACT-STRATEGIST none)))
            false
        )
